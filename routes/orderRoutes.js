...
const Table = require("../models/Table"); // Add this at the top
...
// Get served items and total amount for a specific table
router.get('/served/:tableNumber', async (req, res) => {
  const { tableNumber } = req.params;
  ...
  try {
    ...
    console.error("Error fetching served items:", err);
    res.status(500).json({ message: "Internal server error" });
  }
});
...
// DELETE /orders/clear/:tableNumber
router.delete("/clear/:tableNumber", async (req, res) => {
  try {
    const tableNumber = req.params.tableNumber;
    await Order.deleteMany({ tableNumber: tableNumber });
    res.json({ message: `Cleared orders for table ${tableNumber}` });
  } catch (err) {
